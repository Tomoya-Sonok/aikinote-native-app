...
| iOS deployment target | `16.4` | `expo-build-properties` プラグイン経由で指定。WebView 経由でロードする Web 版の Next.js 16 ランタイムに ES2022 class static blocks が含まれており、これをパースできる Safari 16.4 以降が必須 |

### iOS 最低 OS バージョンを 16.4 に設定している理由

Next.js 16 の公式最低対応ブラウザが Safari 16.4 で、プリコンパイル済みランタイムチャンク（例: `AppRouterContext` を参照する ErrorBoundary クラス）が `class C { static { ... } }` という ES2022 の class static block を含む。Safari 15 系はこれをパースできず `SyntaxError: Unexpected token '{'` でチャンクロードに失敗する。

失敗すると React hydration が走らず、以下のような症状になる:
- ボタン (`<button onClick>`) が全て無反応
- `__AIKINOTE_NATIVE_APP__` 判定ロジックが効かず、ネイティブ専用の UI 抑制（例: ロゴの非リンク化）が動作しない
- `<Link>` は SSR で `<a href>` として吐かれるため、リンク遷移だけは生きる

対策として `expo-build-properties` で `ios.deploymentTarget: "16.4"` を明示し、ストア側で iOS 15.1〜16.3 の端末を弾く。実機テストは iOS 16.4 以降の Simulator / 実機で実施すること。iPadOS 15 系の iPad（iPad Air 2 等）では動作確認できない。
